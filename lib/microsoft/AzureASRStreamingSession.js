...
const RegExpEOS_1 = require("../asr/RegExpEOS");
class AzureASRStreamingSession {
    constructor(config, log) {
        ...
        if (config.regexpEOS && config.regexpEOS.length > 0) {
            this.regexpEOSRegex = RegExpEOS_1.RegExpEOS.buildRegex(config.regexpEOS);
        }
        ...
        this.recognizer.recognizing = (_, eventArgs) => {
            ...
            this.lastASRResult = {
                ...
                confidence: INCREMENTAL_CONFIDENCE,
            ...
            if (!this.stopped) {
                if (this.regexpEOSRegex && this.regexpEOSRegex.test(eventArgs.result.text)) {
                    this.log.debug(`Incremental transcription contains a RegExpEOS trigger word/phrase. Stopping ASR and returning.`);
